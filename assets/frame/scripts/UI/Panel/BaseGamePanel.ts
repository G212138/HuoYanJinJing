...
export default class BaseGamePanel extends BaseUI {
    ...
    start() {
        ...
        EditorManager.hasResult = this.isRresult;
    ...
    protected onRecoveryData(recovery: SyncData) {
        ...
        if (SyncDataManager.syncData.frameSyncData.showGameOver) {
    ...
    protected showGameOverPanel() {
        ...
        console.log("====SyncDataManager.syncData.frameSyncData.hasReplayCount", SyncDataManager.syncData.frameSyncData.hasReplayCount);
        ...
        if (ReportManager.isPassGame() && ReportManager.isPassAdditonal()) {
            SyncDataManager.syncData.frameSyncData.isGameOver = true; // 游戏是否结束
        }
        if (EditorManager.editorData.isStarCount) {
    ...
    private addSDKEventListener() {
        ...
        T2M.addSyncEventListener(FrameMsgType.ENTER_ADDITION, this.onEnterAdditional.bind(this));
